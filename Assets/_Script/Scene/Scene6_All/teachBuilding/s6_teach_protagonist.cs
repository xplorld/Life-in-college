...
public class s6_teach_protagonist : MonoBehaviour {
	...
	void Start () {
		...
		//save scene
		global_data.current_scene = "Scene_6_TeachingBuilding";
	...
	void FixedUpdate(){
		...
		//set position
		if (setPos) {
			if (global_data.teachBuilding == 1) {
				...
				standDirection = 1;
			} else if (global_data.teachBuilding == 0) {
				rb.position = new Vector3 (
					-7.2f,
					-1.3f,
					0.0f
